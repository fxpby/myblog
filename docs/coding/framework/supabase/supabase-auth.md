...
当时光梳理业务逻辑整理业务流程图就输出 30 余张，这还只是前端业务。自己当时带着 3 个前端同事一起从需求分析、旧项目业务梳理、概要设计、详细设计、开发编码提测到第一批次白名单用户上线加班做了快 3 个月。很难想象吧，一个登录需求可能在别人眼里几天最多一周的工作量，但是企业级项目的复杂程度和历史包袱就是如此沉重冗杂
