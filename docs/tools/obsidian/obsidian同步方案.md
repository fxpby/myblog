...
| 自建存储     | 各端笔记在第三方插件的配合下，上传到私有储存空间/其他设备从私有空间下载并更新本地文件                | 本地 NAS/ 云 OSS                | 零成本/ 低成本                | 需购买 / 租用硬件/ 有学习成本  | ⭐⭐⭐⭐ | NAS 用户 / 程序员   | 适合 NAS 玩家和程序员；笔者莫得 NAS，但使用云 OSS 服务，需将图片转图床链接，有额外心智负担，暂不考虑                                                |
...
| 点对点同步   | 各端笔记在同步软件的配合下将笔记文件直接转发给局域网内经过认证的设备                                 | Syncthing/ self-hosted liveSync | 免费/无流量容量限制/ 版本控制 | 公网同步问题/- iOS 支持不好    | ⭐⭐⭐⭐ | 程序员 / 普通用户   | 笔者是 iOS 用户，所以排除不支持 iOS 的方案；网上教程少，官网文档结合教程折腾了很久才部署成功，使用上多设 备数据同步有问题，无奈了，浅浅记录部署流程 |
